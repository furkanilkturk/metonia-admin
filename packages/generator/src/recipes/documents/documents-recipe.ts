...
function documentReplacements(
	...
	return {
		...
		DOCKER_GUIDE: config.docker
			? `## Docker\n\nSet POSTGRES_PASSWORD in a local .env file, then use \`docker compose up --build\`. Compose passes the password separately through PGPASSWORD instead of interpolating it into DATABASE_URL, so URL-significant characters remain valid. The stack health-checks PostgreSQL, applies committed Drizzle migrations in a one-shot service, and starts the Node adapter output only after migration succeeds. Credentials are not baked into the image.`
